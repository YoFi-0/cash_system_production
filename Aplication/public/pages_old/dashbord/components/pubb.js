"use strict";
const _filtter_pupp = ({ input, title, id, styles, onSbmit }) => {
    var thisFather;
...
const _thisStore_filtter_PaymentsPupp_input = () => _thisStore_filtter_PaymentsPupp.elm({}, async (elm) => {
    ...
    const request_lifter_user_id = inputHolder[0].children[1].value.trim();
    const request_lifter_user_name = inputHolder[1].children[1].value.trim();
    const payeer_user_id = inputHolder[2].children[1].value.trim();
    const payeer_user_name = inputHolder[3].children[1].value.trim();
    const total_amount = inputHolder[4].children[1].value.trim();
    const from_date = inputHolder[5].children[1].children[0].children[1].value.trim();
    const to_date = inputHolder[5].children[1].children[1].children[1].value.trim();
...
const _donations_filtter_Pubb_input = () => _donations_filtter_Pubb.elm({}, async (elm) => {
    const inputHolder = elm.element.children[0].querySelectorAll(".input_lable");
    const donater_user_id = inputHolder[0].children[1].value.trim();
    const donater_user_name = inputHolder[1].children[1].value.trim();
    const amount = inputHolder[2].children[1].value.trim();
    const from_date = inputHolder[3].children[1].children[0].children[1].value.trim();
    const to_date = inputHolder[3].children[1].children[1].children[1].value.trim();
...
const my_Store_Payments_filtter_pubb_input = () => my_Store_Payments_filtter_pubb.elm({}, async (elm) => {
    ...
    const payeer_user_id = inputHolder[2].children[1].value.trim();
    const payeer_user_name = inputHolder[3].children[1].value.trim();
    const store_server_id = inputHolder[4].children[1].value.trim();
    const store_server_name = inputHolder[5].children[1].value.trim();
    const total_amount = inputHolder[6].children[1].value.trim();
    const from_date = inputHolder[7].children[1].children[0].children[1].value.trim();
    const to_date = inputHolder[7].children[1].children[1].children[1].value.trim();
...
const privte_donations_filtter_pubb_input = () => privte_donations_filtter_pubb.elm({}, async (elm) => {
    ...
    const donater_user_id = inputHolder[0].children[1].value.trim();
    const donater_user_name = inputHolder[1].children[1].value.trim();
    const donated_user_id = inputHolder[2].children[1].value.trim();
    const donated_user_name = inputHolder[3].children[1].value.trim();
    const amount = inputHolder[4].children[1].value.trim();
    const donationType = inputHolder[5].children[1].value.trim();
    const from_date = inputHolder[6].children[1].children[0].children[1].value.trim();
    const to_date = inputHolder[6].children[1].children[1].children[1].value.trim();
    localStorage_create("log_card_2", {
        donater_user_id: donater_user_id,
        donater_user_name: donater_user_name,
...
const My_Servers_Donations_filtter_pubb_input = () => My_Servers_Donations_filtter_pubb.elm({}, async (elm) => {
    const inputHolder = elm.element.children[0].querySelectorAll(".input_lable");
    const donated_server_id = inputHolder[0].children[1].value.trim();
    const donated_server_name = inputHolder[1].children[1].value.trim();
    const amount = inputHolder[2].children[1].value.trim();
    const from_date = inputHolder[3].children[1].children[0].children[1].value.trim();
    const to_date = inputHolder[3].children[1].children[1].children[1].value.trim();
    localStorage_create("log_card_1", {
    ...
    from_user_logs_start.my_servers_donations = 0;
    _("#log_card_1").addChilds(...Render_Log.My_Servers_Donations(reqData));
    return {
        on_done: () => {
            return {
                is_error: false,
...
const _paypalInput = () => {
    return _paypal_pubb.elm({
        ...
        const email = elm.element.children[0].querySelectorAll(".input_lable")[0].children[1];
        const formData = new FormData();
        let on_done = () => {
            return {
                is_error: true,
