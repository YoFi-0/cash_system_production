...
const renderServerConfig = async() => {
    ...
    _page( is_api_render_error ? _div({classes:"inint_page"}) :
        _div(
            ...
            _thisStore_filtter_PaymentsPupp_input(),
            _donations_filtter_Pubb_input(),
            _paypalInput(),
            _div(
                {
                    classes:"cards"
                },
                UserSide(renderData.username, renderData.user_image, renderData.tag, renderData.pay_pal_email ? true : false), pagePathSide(server_data.server_data.server_name,
                server_data.server_data.server_img.endsWith("/null") ? "/images/store.jpg" : server_data.server_data.server_img,
                server_data.server_data.id ? "configaration added" : "no configaration yet", server_data.server_data.id ? "greeen" : 'red', server_data.server_data.rate ? `${server_data.server_data.rate}` : ""), renderData.pay_pal_email ? PayPalSide(renderData.pay_pal_email) : PayPalSide(),
                // paypal_pubb()
            ),
            !server_data.is_bot_added ? 
                _div(
                    {
                        classes:"no_bot"
                    },
                ...
                _div(
                    ...
                    _inputsHolder({
                        ...
                        onSbmit: async(elm) => {
                            ...
                            formData.append("pay_pal", (input_holder[0].children[1] as HTMLInputElement).value.trim())
                            formData.append("target_lang",(input_holder[1].children[1] as HTMLInputElement).value.trim())
                            formData.append("target_logs_channel_id",(input_holder[2].children[1] as HTMLInputElement).value.trim())
                            if((input_holder[3].children[1].children[2] as HTMLInputElement)?.files){
                                formData.append("img",(input_holder[3].children[1].children[2] as HTMLInputElement).files![0])
                            }
                            formData.append("target_basket_embed_description",(input_holder[4].children[1] as HTMLInputElement).value.trim())
                            formData.append("target_invite_link",(input_holder[5].children[1] as HTMLInputElement).value.trim())
                            formData.append("target_discrption",(input_holder[6].children[1] as HTMLInputElement).value.trim())
                            formData.append("target_products_mangers", JSON.stringify(Array.from(input_holder[7].children[1].children[0].children).map(input => (input as HTMLInputElement).value.trim())))
