...
const cardInfo = ({
    ...
    return _div(
        ...
        _img({src:img_src.endsWith("/null") ? "/images/user.jpg" : img_src , style:{border: isHaveBorder ? "solid #FFF max(0.2vw, 0.2em)" : undefined}}),
        _div(
            {
                classes:"content",
                style:{
...
const UserSide = (username:string, user_image:string, tag:string, isPayPalConnected:boolean) => {
    return cardInfo({
        isHaveSpan:{
            tag:tag
        },
...
const pagePathSide = (path:string, server_image:string, text:string, text_color:"red" | "greeen" | "fff", stars?:string) => {
    return cardInfo({
