...
const _donation_logs = ({
    title,
    logs_cards
}:{
    title:LogsTitleType
    logs_cards:YoFiElement[]
}) => {
    i++;
    let midElm:YoFiElement
    let clear_filter:YoFiElement
    let more_button_elm:YoFiElement
    return _div(
        {
            classes:"log_card",
            id:`log_card${i}`
...
const _payment_log = ({
    ...
    log_msg:string
    invoice_id:string
    is_it_payment?:boolean,
    is_it_server?:boolean
}) => {
    // "https://cdn.discordapp.com/icons/938140512647520306/5902f3b9494911ee2dcf8ffb976b02fa.webp?size=96"
    let donElm:YoFiElement
    let loading_elm:YoFiElement
    let isClicked = false
    let productsHolder:YoFiElement
    ...
    const get_final_amount = (theamount:string | number) => {
    ...
    const _to = ({
        ...
        _p("To: ", {classes:"bold"}),
        _p(is_to_me ? "Me" : name, {classes:"not_bold"}, (elm) => {elm.element.title = name}),
        // _img({
        //     src:img_src
        // }),
    )
    return _div(
        ...
        _div(
            ...
            is_it_payment ? _div(
                ...
                _div(
                    ...
                    _p("--------------------------------"),
                    _p(`total : ${get_final_amount(amount)}$`)
                ),
            ) :
            _div(
                {
        ...
        if(isClicked){
            ...
            if(!isProducts_registerd && productsHolder){
                ...
                productsHolder.addChilds(...invoiceProduct.map((value:any) => {
...
const myServerDonationCreateLogsElm = async(midElm:YoFiElement, flters:any, more_button_elm?:YoFiElement) => {
...
const this_Store_Payments = async(midElm:YoFiElement, flters:any, more_button_elm?:YoFiElement) => {
    const formData =  new FormData()
...
const DonationsCreateLogsElm = async(midElm:YoFiElement,flters:any, more_button_elm?:YoFiElement) => {
    const formData =  new FormData()
...
const private_DonationsCreateLogsElm = async(midElm:YoFiElement, flters:any, more_button_elm?:YoFiElement) => {
    const formData =  new FormData()
    formData.append("donater_user_id", flters.donater_user_id || "")
    formData.append("donater_user_name", flters.donater_user_name || "")
...
const store_PaymentsCreateLogsElm = async(midElm:YoFiElement, flters:any, more_button_elm?:YoFiElement) => {
    const formData =  new FormData()
    formData.append("request_lifter_user_id", flters.request_lifter_user_id || "")
    formData.append("request_lifter_user_name", flters.request_lifter_user_name || "")
    formData.append("payeer_user_id", flters.payeer_user_id || "")
