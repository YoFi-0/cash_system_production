...
export default new Custom_id('show_me_my_basket', async({interaction, client}) => {
    ...
    if(!getUserBasket){
        interaction.editReply(_$.custom_id.show_me_my_basket.badMsgs.emptyBasket)
        return
    }
    ...
    .addComponents(
        ...
        .setPlaceholder(`${interaction.user.username} ${_$.custom_id.show_me_my_basket.selector.basket}`)
        .setCustomId('show_product_basket')
        .setOptions(
            ...userBasket.map((value) => {
                return {
                    label: `${value.quantity}x${value.product_name}`,
                    description: `${_$.custom_id.show_me_my_basket.selector.price}: $${value.product_price}`,
