...
export default new Custom_id('create_section', ({interaction, client}) => {
    if(!interaction.isButton()){
        return
    }
    const _$ = lang_obj(get_lang_by_msg(interaction))
    const modal = new discord.ModalBuilder()
    .setCustomId('create_Section__model')
    .setTitle(_$.custom_id.create_section.model.title);

    const input_1 = new discord.TextInputBuilder()
    .setCustomId('create_Section_row_1').setMaxLength(max_len.section_name)
    .setLabel(_$.custom_id.create_section.model.sectionNameLable)
