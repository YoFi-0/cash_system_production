...
export default new Custom_id('delete_product', ({interaction, client}) => {
    if(!interaction.isButton()){
        return
    }
    const _$ = lang_obj(get_lang_by_msg(interaction))
    const modal = new discord.ModalBuilder()
    .setCustomId('delete_product__model')
    .setTitle(_$.custom_id.delete_product.model.title);

    const input_1 = new discord.TextInputBuilder()
    .setCustomId('delete_product_row_1').setMaxLength(max_len.product_name)
    .setLabel(_$.custom_id.delete_product.model.productNameLable)
